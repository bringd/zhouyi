...
`ResultDisplay.tsx` L29 注释 `// 3. AI 解读 · 简易版 (with "查看完整" expand)` 同步改为 `// 3. 解卦 · 简易版`。
...
| 风险 | 概率 | 回滚 |
|---|---|---|
| 用户不认可「解卦」命名 | 低（已直接确认） | 改回原文案，单次提交 |
| AI 第 7 段偶尔不输出 | 中 | 用户感知不到——只是少一段，不报错 |
| AI 第 7 段跑题 | 中 | 不处理；属 AI 质量问题 |
| 触发 demo 配额多消耗 ~80-150 token/次 | 低（每日 5 次额度内） | 不优化；超过 5 次本就走 BYOK |
| 单测文案遗漏 | 中 | 全仓 grep `开始 AI 解读` 应为空 |
