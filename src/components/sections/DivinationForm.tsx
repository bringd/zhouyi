...
export interface DivinationFormProps {
  /** Optional pre-filled numbers (for testing) */
  initialNumbers?: [number | null, number | null, number | null]
  /** Optional pre-filled question */
  initialQuestion?: string
  /** Called with the divination result id after successful divination + save */
  onResult?: (recordId: string) => void
  className?: string
}

/**
 * The three-number divination form (per brainstorming decision J).
 * Layout: question text box + 3 number boxes + 启卦 button.
 */
export function DivinationForm({ initialNumbers, initialQuestion = '', onResult, className }: DivinationFormProps) {
  const [question, setQuestion] = useState(initialQuestion)
  const [numbers, setNumbers] = useState<[number | null, number | null, number | null]>(
  ...
  return (
    <motion.form
      ...
      className={cn('max-w-2xl mx-auto p-6 bg-rice border-2 border-june-bronze rounded-md', className)}
    ...
    >
      ...
      {error && (
        <div className="text-june-red text-sm text-center font-body mb-4">{error}</div>
      )}
