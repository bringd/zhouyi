...
export const feedRouter = new Hono<{ Variables: { userId: string; sessionId: string } }>()
...
feedRouter.post('/', async (c) => {
  ...
  const { mainHexagramId, changedHexagramId, movingLine, question, aiSummary, note } = parsed.data
