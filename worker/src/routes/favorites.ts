...
export const favoritesRouter = new Hono<{ Variables: { userId: string } }>()
...
favoritesRouter.delete('/:hexagramId', async (c) => {
  ...
  const deleted = await db
    ...
    .where(and(eq(favoriteHexagrams.userId, c.get('userId')), eq(favoriteHexagrams.hexagramId, hexagramId)))
