...
describe("POST /api/auth/sms/send", () => {
  ...
  it("writes 6-digit code to session, expires_at = now + 5min", async () => {
    ...
    const rows = await db
      .select({
        smsCode: sessionsTable.smsCode,
        smsExpiresAt: sessionsTable.smsExpiresAt,
      })
      .from(sessionsTable)
    ...
    expect((rows[0]?.smsExpiresAt as Date).getTime()).toBeGreaterThan(
      Date.now(),
    );
  });
});
...
describe("POST /api/auth/sms/verify", () => {
  ...
  it("verifies correct code and upgrades user to registered", async () => {
    ...
    const rows = await runD1Query<{ sms_code: string }>(
      "SELECT sms_code FROM sessions WHERE sms_phone = '13800138000'",
    );
  ...
  it("re-points a second session at the user that already owns the phone", async () => {
    ...
    const codeA = (
      await runD1Query<{ sms_code: string }>(
        `SELECT sms_code FROM sessions WHERE sms_phone = '${phone}'`,
      )
    )[0].sms_code;
    const verifyA = await app.request(
      "/api/auth/sms/verify",
      {
        method: "POST",
        headers: { ...headers, cookie: cookieA },
        body: JSON.stringify({ phone, code: codeA }),
      },
      env,
    );
    ...
    const verifyB = await app.request(
      "/api/auth/sms/verify",
      {
        method: "POST",
        headers: { ...headers, cookie: cookieB },
        body: JSON.stringify({ phone, code: codeB }),
      },
      env,
    );
  ...
  it("rejects an expired code", async () => {
    ...
    await runD1Query(
      `UPDATE sessions SET sms_expires_at = 1000 WHERE sms_phone = '${phone}'`,
    );
