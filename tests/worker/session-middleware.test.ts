...
async function setupApp(): Promise<Hono> {
  ...
  const app = new Hono<{ Bindings: { DB: D1Database }; Variables: { userId: string; sessionId: string; mode: string; remaining: number | null } }>()
...
describe('sessionMiddleware: mode derivation', () => {
  ...
  it('upgraded user email ends in 11-digit = mode:registered', async () => {
    ...
    const second = await app.request('/test', { headers: { cookie: cookie.split(';')[0] } }, env)
