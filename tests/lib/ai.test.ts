...
describe("ai: generateInterpretation (BYOK browser-direct)", () => {
  ...
  it("omits x-api-key header when no key is stored (proxy demo mode)", async () => {
    clearApiConfig();
  ...
  it("parses split SSE events (chunk boundary mid-event)", async () => {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        controller.enqueue(
          encoder.encode('data: {"type":"content_block_delta","delta":'),
        );
        controller.enqueue(
          encoder.encode('{"type":"text_delta","text":"分片"}}\n\n'),
        );
        controller.enqueue(
          encoder.encode(
    ...
    globalThis.fetch = vi
      .fn()
      .mockResolvedValue({ ok: true, body: stream }) as unknown as typeof fetch;
  ...
  it("ignores the apiKey arg (back-compat: still read from localStorage)", async () => {
    ...
    await expect(
      generateInterpretation(sampleInput, null),
    ).resolves.toBeDefined();
    await expect(
      generateInterpretation(sampleInput, ""),
    ).resolves.toBeDefined();
    await expect(
      generateInterpretation(sampleInput, "sk-some-other-key"),
    ).resolves.toBeDefined();
